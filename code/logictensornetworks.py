...
def disjunction_of_literals(literals):
    ...
    if default_tnorm == "product":
        result = tf.sub(tf.ones([literals[0].rows,1]),tf.reduce_prod(tf.sub(tf.ones([literals[0].rows,1]),literals_tensor),1, keep_dims=True))
    if default_tnorm=="yager2":
        result = tf.minimum(tf.ones([literals[0].rows,1]),tf.sqrt(tf.reduce_sum(tf.square(literals_tensor),1, keep_dims=True)))
    ...
    if default_aggregator == "pmean":
        default_p_pmean_tensor = tf.constant(default_p_pmean, shape=[lit.rows])
        return tf.pow(tf.mul(tf.reduce_sum(tf.pow(result, default_p_pmean_tensor), keep_dims=True),tf.inv(tf.to_float(tf.size(result)))),tf.inv(default_p_pmean))
    if default_aggregator == "hmean":
        return tf.mul(tf.to_float(tf.size(result)), tf.inv(tf.reduce_sum(tf.inv(result), keep_dims=True)))
...
def prod_of_domains(order,domains):
    ...
    if len(order) == 1:
        ...
        return tf.concat(1,[d1,prod_of_domains(order[1:],domains)])
...
class Domain_union(Domain):
    ...
    def __init__(self,domains):
        ...
        self.tensor = tf.concat(0,[dom.tensor for dom in domains])
...
class Function(Domain):
    def __init__(self, label, domains, ranges, value=None):
        ...
        if self.value:
            ...
            self.tensor = tf.add(tf.squeeze(tf.batch_matmul(expandedX,multipleM)),self.n)
...
class Predicate:
    def __init__(self, label, domains):
        ...
        self.V = tf.Variable(tf.zeros([default_layers,
                                               self.columns]),
                             name = "V"+label)
        self.b = tf.Variable(tf.neg(tf.ones([1,default_layers])),
                             name = "b"+label)
    ...
    def tensor(self,order=None,domains=None):
        ...
        Wflat = tf.concat(1,tf.unpack(self.W))
        XW = tf.matmul(X,Wflat)
        XWX = tf.squeeze(tf.batch_matmul(tf.reshape(tf.expand_dims(XW,1),
                                                    [X_rows,
    ...
    def __init__(self,polarity,predicate,order=None,domains=None):
        ...
        if not order and not domains:
            ...
            if polarity:
                ...
                self.tensor = tf.sub(tf.ones([self.rows,1]),predicate.tensor())
        else:
            self.domains = domains
            self.order = order
            self.rows = np.prod([d.rows for d in self.domains])
            ...
            if polarity:
                ...
                self.tensor = tf.sub(tf.ones([self.rows,1]),self.predicate.tensor(order,domains))
    ...
    def __init__(self,label,clauses,save_path=""):
        ...
        if not self.clauses:
            ...
            if default_clauses_aggregator == "wmean":
                ...
                self.tensor = PR(tf.div(tf.reduce_sum(tf.mul(weights_tensor, clauses_value_tensor)),tf.reduce_sum(weights_tensor)))
        self.loss = tf.sub(tf.add(smooth(self.parameters),
    ...
    def penalize_positive_facts(self):
        ...
        for cl in self.clauses:
            for lit in cl.literals:
                tensor_for_positive_facts.append(tf.reduce_sum(Literal(True,lit.predicate,lit.order,lit.domains).tensor,keep_dims=True))
